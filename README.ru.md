...
**Проверки схемы**
- Дефекты структуры, которых не видно на рантайм-страницах: кончающаяся последовательность, таблица без первичного ключа перед включением логической репликации, забытая `UNLOGGED`-таблица, схема, в которой `PUBLIC` может создавать объекты (CVE-2018-1058)
- Читается только системный каталог — пользовательские данные не затрагиваются, это же аргумент для запуска на проде
- Три уровня (`error` / `warning` / `notice`) назначаются в Go по фактам из запроса, поэтому пороги меняются без правки SQL; находки по партициям схлопываются до родительской таблицы и не топят отчёт
- Проверка, которая не смогла выполниться (нет прав, неподдерживаемая версия, таймаут), попадает в список пропущенных с причиной: непроверенная схема никогда не выглядит чистой
- Сводка по инстансу: счётчики по каждой настроенной базе, а не только по выбранной
- Эвристические и шумные проверки (UUID в `varchar`, таблицы вне внешних ключей) подавляются конфигом, самые шумные выключены до явного включения
- Каждая проверка, у которой уже была своя страница — невалидные ограничения, несовпадение типов во внешних ключах, nullable-ключи, похожие ключи и индексы, B-tree по массивам — питает тот же отчёт через единый реестр, поэтому страница и отчёт не расходятся
- Проверки имён — только про то, что ломает инструменты (зарезервированное слово в имени, символы, требующие кавычек), а не про стиль; непроверенные (`NOT VALID`) ограничения со страницы анализа FK теперь тоже попадают в отчёт
- На репликах раздел скрыт — схема там та же, но чинить её можно только на мастере
...
Страница `/schema-lint` работает без настройки. Глобальная секция `schema_lint` нужна, чтобы погасить
то, что в конкретном парке является шумом, и подстроить пороги последовательностей:
...
schema_lint:
  ...
  sequence_thresholds:                       # процент оставшихся значений
    ...
    warning: 10
    notice: 20
  cache_ttl: 5m                              # кнопка «Обновить» на странице игнорирует кэш
```
...
(MIT, © Nikonov, 2024) — набора проверок структуры БД для PostgreSQL. Логика адаптирована под Dasha:
уровень назначается вне SQL, находки по партициям схлопываются, системные схемы отсекаются единообразно.
Пофайловая атрибуция — [backend/internal/query/README.md](backend/internal/query/README.md).
