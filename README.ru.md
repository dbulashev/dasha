...
Демон использует тот же `dasha.yaml`. Все настройки (триггеры, пороги, ретеншен) хранятся в storage DB и редактируются из UI (меню *Авто-снимки*, только admin). Можно запускать в нескольких репликах — `pg_try_advisory_lock` в storage DB выбирает одного лидера.
