...
export interface HealthScore {
  ...
  source?: string
  /** True when source is "metrics" but no datasource series matched any selector for the target — the score is built from absent signals, which read as healthy, so it may be overstated and understate the problem. Check selector label matching at GET /api/common/health-score/datasource/status. */
