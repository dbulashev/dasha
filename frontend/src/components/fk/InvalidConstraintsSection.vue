...
<template>
  <v-card class="mb-4">
    ...
    <v-card-text>
      <v-data-table :headers="headers" :items="items" :loading="loading" density="compact" multi-sort disable-pagination hide-default-footer :no-data-text="t('fk.noInvalidConstraints')" />
    </v-card-text>
  </v-card>
</template>
