...
- **Горячие таблицы и индексы (`GET /api/tables/hot`, `/api/indexes/hot`, `/api/hot/object-history`, `/api/hot/percentile`; MCP-инструменты `hot_tables` / `hot_indexes`):** отвечает на вопрос *что нагружает базу* по периодическим дельта-снимкам счётчиков активности, хранимым так же, как PostgreSQL хранит статистику столбцов: **точный top-N на каждый класс метрик** (чтения / записи / физический I/O; у индексов класса записей нет — PostgreSQL не ведёт таких счётчиков) плюс **дециль-гистограмма хвоста** — вместо полной поимённой истории (растёт со схемой клиента) или top-k-метрик (молча нерепрезентативны). Гистограмма даёт три вещи, которых нет у голых топов: **coverage** (какую долю всей активности покрывает сохранённый топ — чип в секции, чтобы топ не выдавали за полную картину), **перцентиль любого объекта** (живая дельта от якорного среза, спроецированная на распределение хвоста — блок «Активность» в карточке таблицы: сначала абсолютный темп, потом «активнее ~N% таблиц», потому что на тихой базе копеечная активность обгоняет спящий хвост) и ограниченный по построению объём хранения. Счётчики снимаются **со всех хостов кластера** и суммируются (они не реплицируются — урок unused-индексов, применённый к горячим объектам), с разбивкой по хостам и бейджами мастер/реплика в каждой строке топа; недоступный хост делает снимок частичным, а не блокирует его. Съём выполняет autosnapshot-демон по **cron-расписанию** (`hot_schedule`, по умолчанию ежедневно в 03:00 **UTC** — зона зафиксирована явно, иначе голая cron-спека срабатывала бы в зоне контейнера бэкенда; поддержан префикс `CRON_TZ=<зона>`), настраивается вместе с размером топа и ретеншном в UI настроек автоснимков; пропущенные срабатывания забираются на следующем тике демона. Все темпы нормируются на **фактическое окно** снимка, поэтому часовые и пятиминутные расписания честно экстраполируются в «за сутки» — включая перцентиль, который масштабирует живой темп обратно в единицы окна перед проекцией. Хранилище не подвержено bloat по построению: исторические таблицы — append-only суточные партиции со своим возрастным ретеншном (`hot_retention_days`, независим от размерного ретеншна pgss), а единственная перезаписываемая таблица (якорный срез) обновляет только неиндексированные колонки при fillfactor 70 — ежесуточная перезапись остаётся HOT. UI: секции «Горячие таблицы» / «Горячие индексы» (селектор класса, выбор снимка, тренд-стрелки против предыдущего снимка, раскрываемые строки по хостам, сводка хвоста с мини-барами децилей), большие числа сокращаются суффиксами k/M/B. Требуется snapshot-хранилище (`501` без него); секции сами скрываются при его отсутствии. Правила честности на тихих базах: объекты с нулевой активностью никогда не добивают топ («входит в топ горячих», ничего не делая — ровно это наблюдалось на демо-лабе, где 7 мест на 17 в основном спящих таблиц заполнялись тишиной), полностью спящий хвост сворачивается в одну фразу вместо девяти нулевых баров, а блок активности спящей таблицы — в одну строку «заметной активности нет». Страница авто-снимков разделяет два семейства снимков: вкладка настроек группирует триггерные pgss/lock-секции отдельно от секции hot-объектов по расписанию, а статусная строка показывает *последнее триггерное событие* и *последний hot-снимок* как разные факты. (`GET /api/maintenance/autovacuum-summary`):** pie-chart таблиц, превысивших пороги срабатывания автовакуума/автоанализа — с разбивкой *только VACUUM / только ANALYZE / оба*, по собственной формуле PostgreSQL (`threshold + scale_factor × строк`, per-table reloptions учтены) — плюс счётчики работающих сейчас процессов обслуживания (`pg_stat_progress_vacuum` / `pg_stat_progress_analyze`). Рядом с диаграммой — легенда с числами и долями (когда всё в пределах порогов, она скрывается целиком, а вместо диаграммы — зелёное «автовакууму нечего делать»); цвета сегментов прошли CVD-валидацию отдельно для светлой и тёмной темы.
