...
### Фичи
- **Персональные API-токены (PAT):** залогиненный пользователь выпускает персональные API-токены (`POST`/`GET`/`DELETE /api/auth/tokens` + диалог *Персональные API-токены* в меню пользователя), передаваемые в заголовке `X-API-Key` — bearer-креды для не-браузерных клиентов (скрипты, будущий MCP-сервер) с сохранением роли (Casbin RBAC). Хранятся в виде хеша в БД снимков (таблица `api_tokens`, создаётся `dasha migrate`); least-privilege (запрашиваемая роль ≤ своей, по умолчанию `viewer`), anti-chaining (нельзя выпустить из-под PAT), одноразовый показ секрета, мгновенный отзыв, опциональный срок. Управление дополнительно закрыто ролёвкой `auth.pat_min_role` (`admin` по умолчанию, пока фича в обкатке; `viewer` открывает её всем залогиненным). Резолв токена кэшируется с коротким TTL, чтобы не ходить в БД на горячем пути авторизации; `last_used` обновляется не чаще раза за интервал на токен.
- **Вердикт по неиспользуемым индексам (`GET /api/indexes/unused-report`, MCP-инструмент `unused_index_report`):** отвечает на вопрос *можно ли удалить этот индекс?*, а не вываливает сырые счётчики сканов — потому что счётчик сам по себе на этот вопрос не отвечает. Наивная проверка `idx_scan == 0` опасна по двум причинам, и обе теперь учтены: (1) `idx_scan` — статистика **инстанса, она не реплицируется**, поэтому индекс, простаивающий на мастере, может обслуживать всю читающую нагрузку на реплике — отчёт **не принимает instance** и опрашивает все хосты кластера; (2) `idx_scan` — счётчик *с момента последнего сброса статистики*, поэтому ноль сканов через пять минут после `pg_stat_reset()` не значит ничего, а пять сканов за два года означают «не используется» — отчёт читает `pg_stat_database.stats_reset` рядом со счётчиком и нормализует в **сканы в сутки на известном окне** (та же нормализация, что уже применяется в правилах анализа настроек). Каждый индекс возвращается с вердиктом и обоснованием: `drop_candidate` (ноль сканов на всех хостах за адекватное окно), `used` (с именем хоста — включая ключевой случай *«простаивает на мастере, но горяч на реплике»*), `stale_evidence` (сканов так мало, что они могут быть историческими — счётчик не скажет *когда* они были, поэтому сбросить статистику и понаблюдать полный бизнес-цикл), `insufficient_data` (окно слишком короткое — единственным потребителем индекса может быть месячный отчёт) и `unknown`. Недоступный хост даёт `unknown`, а не ложное «не используется»: неполная картина не может оправдать `DROP`. Уникальные и обслуживающие констрейнты индексы исключены полностью — их `idx_scan` не растёт при проверке ограничения, поэтому доказать их ненужность он не может. **Партиционированные индексы судятся целиком:** `pg_stat_user_indexes` не показывает родителя (relkind `I`) — только его дочерние индексы по партициям, поэтому холодная партиция даёт ноль сканов, и это partition pruning работает как задумано, а вовсе не мёртвый индекс. Хуже того, дочерний индекс вообще нельзя удалить (`ERROR: cannot drop index … because index … requires it`), а подсказка PostgreSQL указывает на *родителя*, удаление которого снесёт индекс со **всех** партиций, включая горячие. Поэтому отчёт рекурсивно поднимается по `pg_inherits` к корневому индексу (партиция сама может быть партиционированной), суммирует сканы потомков по всем партициям и всем хостам и называет целью родителя как единственную удаляемую единицу: одной работающей партиции достаточно, чтобы сохранить весь индекс. Контракт существующего `/api/indexes/unused` не изменён.
- **MCP-коннектор (`dasha-mcp`):** отдельный read-only [MCP](https://modelcontextprotocol.io)-сервер поверх Dasha API для AI-ассистентов (Claude Desktop / Code, opencode, Cursor, …). 24 read-only tools (кластеры, ранжирование здоровья флота, instance info, health score/тренд/по-базам и рекомендации — плюс `health_details`, drill-down по правилу, превращающий рекомендацию в конкретную цель: передайте её `rule_id` как `detail`, и он назовёт сами объекты (таблицы с выключенным автовакуумом, таблицы с низкой долей HOT-обновлений, таблицы с высокой долей мёртвых кортежей, базы у горизонта xid-wraparound, сессии, удерживающие горизонт xmin), поскольку сама рекомендация сообщает лишь правило и счётчик, top/активные/блокированные запросы, отчёт по запросам и сравнение снимков, индексы, крупнейшие таблицы, детальный разбор таблицы, репликация, анализ настроек, wait events, соединения, риск xid-wraparound, поиск по логам Yandex Cloud — с учётом rate limit: dedup по умолчанию, локальная валидация аргументов, чтобы невалидный запрос не сжигал слот лимита, а 429/501/504 переводятся в понятные модели подсказки) и 5 prompts (`diagnose_cluster`, `explain_health_score`, `find_index_opportunities`, `investigate_slow_queries`, `fleet_overview`). На каждом tool — MCP-аннотации read-only + closed-world, чтобы клиенты могли авто-аппрувить; сервер отдаёт инструкции по использованию для модели, а HTTP-режим шарит один кэш схем между per-token серверами. Работает по **stdio** (одна личность) или **HTTP/SSE** с per-user passthrough токена (общего серверного токена нет — Casbin RBAC сохраняется). Хардненинг: лимит размера ответа tool (слишком большой результат отклоняется, не режется в невалидный JSON), опциональные `limit`-аргументы у `describe_table` (партиции) и `connections` (сэмпл активности), хэшированный и ограниченный кэш серверов по токену, токены не логируются; rate-limit действует через вышестоящий per-identity лимитер Dasha. Двуязычная (en/ru, `--lang` / `DASHA_MCP_LANG`) встроенная база знаний в виде 3 MCP-ресурсов (`dasha://kb/health-rules` — каждое правило скоринга с порогами LOW/MED/HIGH и первыми действиями, синхронность с движком правил гарантирует sync-тест; `dasha://kb/wait-events` — глоссарий wait events; `dasha://kb/workflow` — сценарии «жалоба → цепочка» и правила бережности к API), а 5 prompts — линейные плейбуки (нумерованные шаги, один tool на шаг, критерий трактовки на каждом) — чтобы LLM без глубокой экспертизы PostgreSQL могли грамотно пользоваться инструментами, не замедляя сильные модели. Эксплуатационный паритет с бэкендом: каждый MCP-вызов логируется через zap в stderr (метод, tool, длительность — никогда аргументы или токены), HTTP-режим корректно дожидается in-flight запросов по SIGTERM/SIGINT, версия сервера прошивается из релизной сборки (та же схема internal/version + ldflags, что у бэкенда). Типизированный клиент к Dasha API из OpenAPI-спеки; multi-arch образ `deploy/images/Dockerfile.mcp`, публикуется в release-workflow; опциональный gated Helm Deployment + Service (`mcp.enabled`); чарт намеренно не предлагает общий fallback-токен — HTTP-режим строго per-user passthrough.

## v1.4.0

### Фичи
- **Поиск по логам Yandex Cloud (новая верхнеуровневая страница `/logs`):** для кластеров, обнаруженных через service discovery Yandex MDB, — поиск и просмотр логов сервера PostgreSQL и пулера соединений (Odyssey) через API Yandex MDB.
  - Новый эндпоинт `GET /api/logs` (`getLogs`). Бэкенд читает `StreamClusterLogs` как ограниченное историческое чтение (заданы `from`/`to`), поэтому можно запрашивать прошедшие интервалы, а не только «хвост» в реальном времени.
  - **Нативные серверные фильтры** по `severity` и `host` (единственные поля, по которым фильтрует API Яндекса); подстрока `message`, `database` и `user` фильтруются на стороне Dasha поверх потока. Выражение нативного фильтра собирается только из allowlist (enum severity + проверенные хосты кластера), поэтому защищено от инъекций. Регистр severity зависит от сервиса: PostgreSQL `ВЕРХНИЙ` (`error_severity`), пулер `нижний` (`level`).
  - **Опциональная дедупликация** группирует почти одинаковые сообщения по нормализованному тексту с `count` + `first_seen` / `last_seen` и представительным (наиболее тяжёлым) severity.
  - **Курсорная пагинация** (`next_page_token`) для недедуплицированных результатов с кнопкой «Загрузить ещё». Токен выдаётся только когда следующее совпадение реально существует (поток дочитывается за пределы заполненной страницы), поэтому «Загрузить ещё» никогда не возвращает пустую страницу; при достижении лимита сканирования (`max_scan`) показывается баннер `partial`. Комбинация `page_token` с `dedup` отклоняется (`400`), так как курсор возобновления молча занижал бы счётчики групп.
  - **Частичные результаты при таймауте:** если чтение из API превышает `timeout_seconds`, уже собранные записи (или dedup-группы) возвращаются как частичная страница с флагом `partial` вместо голого `504`.
  - Чувствительный текст маскируется через `sanitize.SQL()` (с учётом типа сервиса) до выхода из бэкенда; ключи сервисного аккаунта не покидают бэкенд (переиспользуются из discovery через внутренний реестр SDK).
  - Доступ — `viewer+` (покрыт существующей политикой `GET /api/*`). Кластеры объявляют возможность поиска логов новым полем `supports_logs` в объектах `Cluster` (рядом с `source`); пункт меню `Логи` появляется только при наличии хотя бы одного такого кластера, а `GET /api/logs` возвращает `501` для кластеров без поддержки поиска логов.
  - Новый глобальный параметр конфига `log_search` (`max_scan` по умолчанию 5000, `max_page_size` по умолчанию 1000, `timeout_seconds` по умолчанию 30).
  - **Гистограмма частоты логов на `/logs`:** stacked bar «время × severity» по загруженным результатам, считается на клиенте — без дополнительных обращений к API Яндекса. Бакеты покрывают фактический диапазон загруженных записей (покрытие указано в подписи); цвета severity проверены на CVD-различимость для светлой и тёмной темы. Только хронологический режим (у dedup-групп нет таймстемпов отдельных записей).
  - **Пресеты поиска и deep links:** восемь пресетов в один клик (autovacuum, дедлоки, чекпойнты, временные файлы, отменённые запросы, ошибки подключения, медленные запросы, ошибки); у рекомендации health-score про дедлоки — кнопка «Найти в логах» → `/logs?preset=deadlock` для кластеров с поиском по логам.
  - **UX фильтров:** несколько include-подстрок (И) и несколько исключений `grep -v`; исключение с плейсхолдером `<*>` матчится по маскированному шаблону — один клик исключает всю форму dedup-группы; клик по severity-чипу и значениям user/database/host в раскрытой строке фильтрует по ним; фильтры синхронизируются с URL (шаримые ссылки) и сохраняются в localStorage (конкретные даты не восстанавливаются); Enter запускает поиск; красная точка на кнопке «Искать» — фильтры изменены после последнего поиска; редко используемые хост/БД/пользователь убраны под спойлер, который сам раскрывается при заполнении.
  - **Interop периода и зум гистограммы:** копирование/вставка периода в формате буфера тайм-пикера Grafana (`{"from":"now-1h","to":"now"}` или абсолютные локальные даты); клик по столбцу гистограммы или drag-выделение диапазона (chartjs-plugin-zoom) задаёт произвольный период в фильтрах — сам поиск остаётся осознанным кликом.
  - **Качество дедупликации:** WAL LSN (`2E/28E36B88`) маскируются при нормализации, поэтому сообщения checkpoint/restartpoint корректно группируются; строка dedup-группы показывает маскированный шаблон (плейсхолдеры `<*>`) вместо конкретных цифр одного из членов.
  - **Детали записи:** `query`/`internal_query` с подсветкой SQL-синтаксиса; меню копирования (все поля / только query / только сообщение / в исключения); нулевые поля (`query_id=0` и т.п.) скрыты.
  - **Отсчёт при 429:** сообщение о превышении лимита показывает живой 30-секундный обратный отсчёт вместо статичного текста.
  - **Health Score:** предупреждение про `wal_level=logical` без логических слотов подавлено для кластеров Yandex MDB (`wal_level` фиксирован платформой и не настраивается); текст рекомендации про место на диске хоста переписан понятнее.
  - **Пер-пользовательский rate limit для `GET /api/logs`:** отдельный от глобального auth-лимита, настраивается через `log_search.rate_limit` / `log_search.admin_rate_limit` (по умолчанию: 1 запрос/30 с и burst 10; для админов 1 запрос/5 с и burst 20; `requests_per_second: 0` отключает). Превышение возвращает `429` с отдельным сообщением на странице `/logs`. Каждый поиск также логируется на уровне info с именем пользователя.

### Зависимости
Обновления Dependabot после v1.3.0 — плановые minor/patch-обновления, если не отмечено иное:
- **Бэкенд (Go):** echo 4.15.2→4.15.4, go-oidc/v3 3.18.0→3.19.0, oapi-codegen/runtime 1.4.1→1.4.2, validator/v10 10.20.0→10.30.3, yandex-cloud/go-genproto 0.84.0→0.92.0 (актуальные описания API YC, используются поиском по логам), testcontainers-go 0.42.0→0.43.0 (только тесты).
- **Фронтенд:** vuetify 3.12.9, vue-i18n 11.4.6, @fontsource/roboto 5.2.10; dev-инструменты: vue-tsc 3.3.6, vitest 4.1.9, eslint 10.6.0, prettier 3.8.4, @playwright/test 1.61.0, @types/jsdom 28.0.3, jiti 2.7.0.
- **Инфраструктура:** базовый образ alpine 3.23→3.24 — security-значимое обновление (свежие фиксы CVE в системных пакетах, держит зелёным Trivy-гейт образа); actions/checkout 6→7.
...
- **Metrics-backed Health Score (опционально):** если настроен Prometheus/VictoriaMetrics-datasource (`health_score.metrics` в `dasha.yaml`), score, рекомендации и новый тренд считаются из time-series метрик (pgSCV + Yandex MDB + pgbouncer + host), а не из точечного SQL. SQL-snapshot остаётся zero-config фолбэком; поле `source` в `GET /api/common/health-score` показывает `"snapshot"` или `"metrics"`.
  ...
  - **Регрессия sequential scans** → `performance`: темп чтения строк seq-scan'ами относительно своей сезонной нормы (час×день недели) — рост сигналит, что индексы перестали использоваться или протухла статистика (ANALYZE), без ложных срабатываний на штатных аналитических сканах.
  - **Свободное место на диске хоста** → `storage`: занято/всего на самой заполненной ФС (из pgSCV `node_filesystem_*` и Yandex Cloud `disk_used_bytes`/`disk_total_bytes`), пороги LOW/MED/HIGH на ≥70/80/90% и **роль-агностичный критический floor на ≥90%** (полный data-том останавливает запись).
...
- **Vacuum в maintenance переписан как очередь по триггеру autovacuum (меньше ложных срабатываний):** категория `maintenance` теперь выводит вакуумные сигналы из собственного триггера autovacuum PostgreSQL, а не из сырого «времени с последнего vacuum». Таблица считается *просроченной*, когда `n_dead_tup` превышает `autovacuum_vacuum_threshold + autovacuum_vacuum_scale_factor·reltuples` либо `n_ins_since_vacuum` превышает insert-порог — с учётом потабличных переопределений в `reloptions` (переиспользована формула из `describe_vacuum_stats`). Крупные статичные / преимущественно читаемые таблицы и секции, которые autovacuum справедливо не трогает, больше не занижают score.
  - Новое правило `vacuum_backlog` + штраф `vacuum_backlog_tables` — длина очереди (таблицы, подлежащие autovacuum прямо сейчас, по dead-кортежам **или** insert-триггеру). Пороги ≥6 / ≥15 / ≥30.
  - `max_vacuum_age_hours` → `max_overdue_vacuum_age_hours`: возраст самого старого vacuum **только среди таблиц из очереди**, поэтому статичные таблицы больше не разгоняют сигнал «застрявшего vacuum».
  - `stale_planner_stats` (auto-analyze) теперь использует тот же reloption-aware порог, консистентно с вакуумной очередью.
  - Вакуумная очередь считается **только из SQL-снимка** (`reltuples` / `n_ins_since_vacuum` / GUC autovacuum нельзя достоверно выразить через time-series); в metrics-режиме она накладывается из снимка, поэтому оба вакуумных правила продолжают влиять и на score, и на рекомендации.
