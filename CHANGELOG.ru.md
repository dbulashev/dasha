...
- **Поиск по логам Yandex Cloud (новая верхнеуровневая страница `/logs`):** для кластеров, обнаруженных через service discovery Yandex MDB, — поиск и просмотр логов сервера PostgreSQL и пулера соединений (Odyssey) через API Yandex MDB.
  ...
  - **Курсорная пагинация** (`next_page_token`) для недедуплицированных результатов с кнопкой «Загрузить ещё»; при достижении лимита сканирования (`max_scan`) показывается баннер `partial`.
  ...
  - Доступ — `viewer+` (покрыт существующей политикой `GET /api/*`). Пункт меню `Логи` появляется только при наличии хотя бы одного кластера `yandex-mdb`.
  ...
  - Объекты `Cluster` в API теперь содержат `source` (`static` | `yandex-mdb`).
