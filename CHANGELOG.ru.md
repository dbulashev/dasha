...
- **Поиск по логам Yandex Cloud (новая верхнеуровневая страница `/logs`):** для кластеров, обнаруженных через service discovery Yandex MDB, — поиск и просмотр логов сервера PostgreSQL и пулера соединений (Odyssey) через API Yandex MDB.
  ...
  - **Пер-пользовательский rate limit для `GET /api/logs`:** отдельный от глобального auth-лимита, настраивается через `log_search.rate_limit` / `log_search.admin_rate_limit` (по умолчанию: 1 запрос/30 с с burst 10; для админов 1 запрос/5 с с burst 20; `requests_per_second: 0` отключает). Превышение возвращает `429` с отдельным сообщением на странице `/logs`. Каждый поиск также логируется на уровне info с именем пользователя.
