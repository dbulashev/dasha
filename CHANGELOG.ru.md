...
### Фичи
- **«I/O»** — новая страница на `pg_stat_io` (PostgreSQL 16 и новее): куда на самом деле уходит дисковый трафик сервера — в разрезе того, кто его создал, к чему обращался и в каком контексте: обычная работа, очистка или массовые операции вроде COPY и больших последовательных чтений. На странице два режима: история по снимкам, которые демон автоснимков снимает с каждого хоста по собственному расписанию, и живой режим с дельтой между тиками, работающий и без хранилища снимков. Карточки сводят эффективность буферного кэша, цену очистки и цену массовых операций; ниже — полная матрица backend_type × object × context. Там, где ряд разорвали сброс статистики, рестарт или мажорный апгрейд, на графике разрыв, а не вводящий в заблуждение ноль. При выключенном `track_io_timing` режим таймингов отключён и объясняет почему. На старых серверах, где `pg_stat_io` нет вовсе, пункт меню отсутствует.
