...
### Фичи
- **Два MCP-инструмента по вводу-выводу** — `io_summary` и `io_trend`: ассистент может спросить, кто читает и пишет — клиентская нагрузка, автовакуум или чекпойнтер — и когда это началось, плюс страница базы знаний о том, как читать счётчики. Пустой ответ называет причину, а период, разорванный сбросом статистики, несёт измеренную часть; нужен PostgreSQL 16 или новее.
