...
### Фичи
- **«Проверки схемы»** — новая страница: дефекты структуры, которые не видны, пока не приведут к аварии, — кончающаяся последовательность, таблица без первичного ключа, забытая `UNLOGGED`-таблица, схема, в которой создавать объекты может кто угодно. Читается только системный каталог, пользовательские данные не затрагиваются. У каждой находки написано, к чему дефект приводит и как его чинить, а где исправление однозначно — готовый SQL для копирования; дефект, найденный на партициях одной таблицы, показывается один раз, на родительской. Проверки, которые не смогли выполниться, перечислены отдельно с причиной, чтобы непроверенная схема не выглядела чистой. Есть сводка по инстансу — счётчики по всем базам, а не только по выбранной. На репликах страница скрыта.
- **Кончающиеся последовательности теперь влияют на health score и без источника метрик.** Раньше рекомендация появлялась только в режиме метрик; теперь она поднимается и по проверкам схемы, по тем же порогам, что на странице, и ведёт к самим последовательностям.
- **Проверки, у которых уже были свои страницы — невалидные ограничения, несовпадение типов во внешних ключах, nullable-ключи, похожие ключи и индексы, B-tree по массивам — теперь попадают и в отчёт по схеме**, так что вопрос «что не так со схемой» закрывается одним разделом. Сами страницы работают как раньше, и на каждой из них теперь видна схема объекта, которой не хватало.
- **Проверки схемы теперь ловят и имена, ломающие инструменты** (зарезервированное слово SQL в имени объекта, символы, требующие кавычек в каждом обращении), и непроверенные `NOT VALID` ограничения, которые раньше были видны только на странице анализа внешних ключей.
