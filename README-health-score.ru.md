...
Функции штрафов считают плавный градиент; в таблице — точки, где меняется наклон.
...
- instance-only категории (`connections`, `replication`, `horizon`, `wal_checkpoint`, `locks`) скрываются при drill-down на конкретную базу;
...
`GET /api/health-score/databases` собирает per-database метрики (cache hit ratio, dead tuples, давность вакуума) в `DatabaseScore` по каждой БД. Движок правил пересчитывается в database-scope, скрывая instance-only категории. В UI таблица сортируется по размеру или score, выбранную базу можно закрепить как контекст рекомендаций.
