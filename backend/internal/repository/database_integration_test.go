...
func TestGetDatabaseHealth(t *testing.T) {
	...
	result, err := p.getDatabaseHealth(ctx, vNum, pool)
	require.NoError(t, err)
	require.NotNil(t, result)

	// On a fresh DB: deadlocks and conflicts should be 0
	assert.GreaterOrEqual(t, result.Deadlocks, int64(0))
	assert.GreaterOrEqual(t, result.Conflicts, int64(0))
	assert.GreaterOrEqual(t, result.XactCommit, int64(0))
	assert.GreaterOrEqual(t, result.XactRollback, int64(0))
	assert.GreaterOrEqual(t, result.RollbackRatio, 0.0)
	assert.LessOrEqual(t, result.RollbackRatio, 1.0)
	// checksum_failures may be nil if data checksums not enabled
}
