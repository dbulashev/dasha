...
func TestGetIndexesBloat(t *testing.T) {
	...
	result, err := p.getIndexesBloat(ctx, vNum, pool, 100, 0, defaultPgStatsView)
	require.NoError(t, err)
	// Bloat may be empty on fresh data — just verify SQL executes and mapping works

	// Test pagination
	page1, err := p.getIndexesBloat(ctx, vNum, pool, 2, 0, defaultPgStatsView)
	require.NoError(t, err)

	page2, err := p.getIndexesBloat(ctx, vNum, pool, 2, 2, defaultPgStatsView)
	require.NoError(t, err)
