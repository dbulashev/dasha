...
autovacuum выключен глобально, выключен track_counts, sequence за порогом
error в `schema_lint` (по умолчанию осталось меньше 5% значений, порог
сдвигается вместе с `schema_lint.sequence_thresholds`), любая ошибка
контрольных сумм, диск хоста ≥90%. Первые
четыре проверяются только на мастере — реплика не запускает autovacuum и не
двигает последовательности. Ошибки контрольных сумм и заполненный диск от роли
не зависят и зажимают любой инстанс.
...
**Поле `database`** в рекомендации — база, в которой искать подтверждение.
Брать её, а не ту базу, о которой спросили. null означает уровень кластера:
ни одна база не владеет находкой. В режиме метрик сигналы из datasource —
агрегаты по всему инстансу, поэтому `low_cache_hit_ratio`,
`high_max_dead_ratio`, `high_avg_dead_ratio`, `low_hot_update_ratio`,
`xid_wraparound_risk`, `checksum_failures`, `host_disk_space`,
`latency_regression` и `seq_scan_regression` там приходят с null, а в
snapshot-режиме — с базой. Не читать null как «проблема ни в одной базе».
У `sequence_exhaustion` на уровне инстанса null в обоих режимах (это худшая
...
также держат горизонт (см. horizon_lag_xids). Перед советом прервать смотреть
backend_type: autovacuum worker штатно работает часами на большой таблице или
при заморозке, прерывание лишь заставит начать заново. В рекомендации указана
база сессии — часто это не выбранная сейчас база.
...
При трафике меньше 50 000 блоков правило не срабатывает — на крошечной базе
показатель ничего не значит. Порогу нужен счётчик блоков из SQL-снимка;
datasource метрик отдаёт только сам показатель, и там правило работает как раньше.
...
### sequence_exhaustion
Худшее использование sequence относительно реально действующего потолка
(колонка-владелец int4 ограничивает bigint-последовательность значением
2147483647). LOW при остатке меньше 20% свободных значений,
MED — меньше 10%, HIGH — меньше 5% (тогда же score зажимается до ≤30) — те же
точки, что у `schema_lint`, и сдвигаются вместе при настройке
`schema_lint.sequence_thresholds`. Но `schema_lint` поднимает уровень на
ступень, если владелец — колонка `integer`, а здесь этого нет: уровень может
быть на единицу ниже, чем в проверках схемы. Данные
берутся из метрик, если настроен источник, иначе читаются из каталога по всем
базам инстанса — то есть это худшая последовательность на инстансе. Расширить
последовательность, а при типе колонки int4 — и саму колонку: это переписывает
таблицу и требует окна. Конкретные последовательности — в `schema_lint`.
...
и завершить/починить. Если это autovacuum worker — прерывать не советовать:
он и есть очистка и отпустит горизонт сам. Горизонт общий для кластера, но в
рекомендации указана база удерживающей сессии.
...
(`search_logs` с пресетом на кластерах с supports_logs=true).
