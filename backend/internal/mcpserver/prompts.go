...
A not_found error names in scope what did not resolve: cluster, instance, database, or feature (the endpoint is disabled in Dasha's configuration). did_you_mean lists similar names with the fields that tell them apart; call again with an exact one. Never swap in a suggestion silently: names ending in -dr, -replica or -preprod differ from the target by a few characters and are different machines.

A result that opens with a {"shaped":…} block was narrowed: the block says what was folded, how many records exist and which parameter returns the full view. If a result is refused as too large, narrow it (one database, a smaller range, or a more specific tool).`,
...
Ошибка not_found называет в scope, что не нашлось: cluster, instance, database или feature (эндпоинт выключен в конфигурации Dasha). В did_you_mean лежат похожие имена с полями, которыми они различаются; повторите вызов с точным именем. Не подставляйте подсказку молча: имена с -dr, -replica или -preprod отличаются от цели парой символов и указывают на другие машины.

Результат, который начинается с блока {"shaped":…}, сужен: блок говорит, что свёрнуто, сколько всего записей и какой параметр вернёт полный вид. Если результат отклонён как слишком большой — сузьте запрос (одна база, меньший диапазон или более специфичный инструмент).`,
