...
Getting oriented:
- Call list_clusters first to pick a cluster (one row per cluster; cluster='words' filters by name), then list_clusters(cluster=<exact name>) for its host and database names; or fleet_health for a worst-first overview of the whole fleet.
...
- "Which index should I create?" is answered by index_advisor, not by list_indexes(kind='missing'): it parses the statements of every cluster host and drops what the existing indexes already serve, and it returns ready DDL. Only a not_found with scope='feature' (the advisor is disabled) sends you to list_indexes(kind='missing') — say then that it reads no queries. weight_pct is the share of analyzed time the covered statements hold, never a predicted speed-up: planner_checked is false. A candidate list that is empty while gaps is non-empty means part of the workload was never analyzed, not that the database is well indexed. Check unused_index_report on the same database before advising a CREATE, and hand the DDL to a human — Dasha never executes it.
- health_trend needs metrics-backed mode (a configured datasource); it returns an error otherwise. Its summary already answers when, how deep and which category: read dips and periods, do not request points to recompute them.
- query_compare needs snapshot IDs from list_snapshots. Its verdict already separates 'more_calls' from 'slower' per call; the full text of a clipped query is in query_report(queryid).
- search_logs works only on clusters with supports_logs=true (see list_clusters) and is rate-limited per user because every call reaches an external log store: combine all filters into one call, keep dedup on, and after a 429 back off instead of retrying immediately.
- "Why is this statement slow, and what plan does it run?" -> plan_insights for a window of the logs, then query_plans(scan_id, query_id) for its trees, plan_regressions for "it got worse" against an earlier window. They need supports_logs=true and auto_explain on the cluster. The plans are only the runs slower than auto_explain.log_min_duration: never present their counts or sum_ms as the workload (top_queries is the workload), and never read a statement's absence as proof it is fast. On a partial window counts are lower bounds and regression ratios must not be quoted as a multiple. An empty result always names empty_reason and configuration says what is switched off — report that, not "no slow queries". Every scan is rate-limited, plan_regressions strictly: refine through scan_id, which reads no logs. In index_advisor, evidence='not_searched' is never an argument against an index — only 'not_found' is.
...
- If unsure how to interpret a result or which tool to call next, read the resources first: dasha://kb/workflow (complaint-to-tool-chain playbooks), dasha://kb/health-rules (rule thresholds and first actions), dasha://kb/schema-checks (schema defect codes and their fixes), dasha://kb/index-advisor (how to read index candidates), dasha://kb/log-plans (how to read plans from the logs), dasha://kb/wait-events (wait event glossary), dasha://kb/pg-stat-io (how to read the I/O counters).

A not_found error names in scope what did not resolve: cluster, instance, database, object (a name the cluster list cannot confirm, such as a table, a snapshot or a database the cluster does not list, does not exist, or the endpoint is disabled) or feature (the endpoint is disabled in Dasha's configuration). did_you_mean lists similar names with the fields that tell them apart; call again with an exact one. Never swap in a suggestion silently: names ending in -dr, -replica or -preprod differ from the target by a few characters and are different machines.

A result that opens with a {"shaped":…} block was narrowed: the block says what was folded, how many records exist and which parameter returns the full view. If a result is refused as too large, narrow it (one database, a smaller range, limit or page_size, or a more specific tool).`,
		...
		indexes: "Find indexing opportunities in database %q of cluster %q instance %q. Execute in order:\n" +
			...
			"On not_found read scope: a cluster or database name is fixed from did_you_mean; only scope=feature " +
			"(the advisor is disabled) sends you to list_indexes (kind=missing) — say plainly that it is a " +
...
С чего начать:
- Сначала list_clusters — выбрать кластер (строка на кластер; cluster='слова' фильтрует по имени), затем list_clusters(cluster=<точное имя>) — имена его хостов и баз; либо fleet_health — обзор всего флота от худших.
...
- На вопрос «какой индекс создать» отвечает index_advisor, а не list_indexes(kind='missing'): он разбирает запросы со всех хостов кластера, отбрасывает то, что уже обслуживают существующие индексы, и отдаёт готовый DDL. На list_indexes(kind='missing') переходите только по not_found со scope='feature' (советник выключен) и тогда так и скажите, что он запросов не читает. weight_pct — доля проанализированного времени, которую занимают покрытые запросы, а не предсказанное ускорение: planner_checked равен false. Пустой список кандидатов при непустом gaps означает, что часть нагрузки не проанализирована, а не что база хорошо проиндексирована. Перед советом «создать» сверьтесь с unused_index_report по той же базе, а DDL отдайте человеку — Dasha его не выполняет.
- health_trend требует режима метрик (настроенный datasource), иначе вернёт ошибку. Сводка уже отвечает, когда, насколько и какая категория: читайте dips и periods, не запрашивайте points, чтобы пересчитать их.
- query_compare требует ID снимков из list_snapshots. Его verdict уже отличает «чаще вызывается» (more_calls) от «медленнее на вызов» (slower); полный текст обрезанного запроса — в query_report(queryid).
- search_logs работает только на кластерах с supports_logs=true (см. list_clusters) и лимитирован per-user, т.к. каждый вызов уходит во внешнее хранилище логов: собирайте все фильтры в один вызов, держите dedup включённым, после 429 выдержите паузу вместо немедленного повтора.
- «Почему запрос медленный и какой у него план?» -> plan_insights за окно логов, затем query_plans(scan_id, query_id) за деревьями, plan_regressions — на «стало хуже» относительно более раннего окна. Нужны supports_logs=true и auto_explain на кластере. Планы — только запуски дольше auto_explain.log_min_duration: не выдавайте их счётчики и sum_ms за нагрузку (нагрузку показывает top_queries) и не считайте отсутствие запроса доказательством, что он быстрый. На неполном окне счётчики — нижняя граница, а кратности регрессий нельзя называть «в N раз». Пустой результат всегда называет empty_reason, а configuration говорит, что выключено, — сообщайте это, а не «медленных запросов нет». Каждый скан лимитирован, plan_regressions строго: уточняйте через scan_id, он логи не читает. В index_advisor evidence='not_searched' никогда не довод против индекса, довод только 'not_found'.
...
- Если непонятно, как трактовать результат или какой инструмент звать дальше — сначала прочитайте ресурсы: dasha://kb/workflow (сценарии «жалоба -> цепочка»), dasha://kb/health-rules (пороги правил и первые действия), dasha://kb/schema-checks (коды дефектов схемы и их лечение), dasha://kb/index-advisor (как читать кандидатов на индексы), dasha://kb/log-plans (как читать планы из логов), dasha://kb/wait-events (глоссарий wait events), dasha://kb/pg-stat-io (как читать счётчики I/O).

Ошибка not_found называет в scope, что не нашлось: cluster, instance, database, object (в вызове названа таблица, снимок или база, которых список кластеров не перечисляет: их нет, либо эндпоинт выключен) или feature (эндпоинт выключен в конфигурации Dasha). В did_you_mean лежат похожие имена с полями, которыми они различаются; повторите вызов с точным именем. Не подставляйте подсказку молча: имена с -dr, -replica или -preprod отличаются от цели парой символов и указывают на другие машины.

Результат, который начинается с блока {"shaped":…}, сужен: блок говорит, что свёрнуто, сколько всего записей и какой параметр вернёт полный вид. Если результат отклонён как слишком большой — сузьте запрос (одна база, меньший диапазон, limit или page_size, более специфичный инструмент).`,
		...
		indexes: "Найди возможности для индексов в базе %q кластера %q, инстанс %q. Выполняй по порядку:\n" +
			...
			"и warnings. При not_found смотри scope: имя кластера или базы исправляй по did_you_mean; только " +
			"scope=feature (советник выключен) ведёт на list_indexes (kind=missing), и тогда прямо скажи, что это " +
			"эвристика по pg_stat_user_tables, которая запросов не читает.\n" +
