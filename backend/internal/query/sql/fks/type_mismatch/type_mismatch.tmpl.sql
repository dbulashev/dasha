WITH fk_with_attributes AS (SELECT c.conname              as fk_name,
                                   c.conrelid,
                                   c.confrelid,
                            ...
                            FROM pg_catalog.pg_constraint AS c
                                     ...
                                     LEFT JOIN pg_catalog.pg_attribute AS rel_att
                                               ON rel_att.attrelid = c.conrelid AND
                                                  rel_att.attnum = fk_conkey.conkey_number
...
FROM (SELECT fk_name,
      ...
      WHERE ((rel_att_type_id <> frel_att_type_id) OR (rel_att_type_mod <> frel_att_type_mod))
      GROUP BY 1, 2, 3) AS c
