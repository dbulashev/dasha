WITH stst as (
    SELECT
        ...
        sum(pss.shared_blk_read_time) AS blk_read_time,
        sum(pss.shared_blk_write_time) AS blk_write_time,
        sum(pss.temp_blk_read_time) AS temp_blk_read_time,
        sum(pss.temp_blk_write_time) AS temp_blk_write_time,
    ...
    GROUP BY pss.queryid, pss.dbid
     ...
     stst_ AS (
         SELECT
             ...
             s.blk_read_time + s.blk_write_time + s.temp_blk_read_time + s.temp_blk_write_time AS io_time,
             100.0 * (s.blk_read_time + s.blk_write_time + s.temp_blk_read_time + s.temp_blk_write_time) / nullif( sum(s.blk_read_time + s.blk_write_time + s.temp_blk_read_time + s.temp_blk_write_time) OVER (PARTITION BY s.dbid) , 0) AS io_time_pct,
             100.0 * (s.blk_read_time + s.blk_write_time + s.temp_blk_read_time + s.temp_blk_write_time) / nullif( sum(s.blk_read_time + s.blk_write_time + s.temp_blk_read_time + s.temp_blk_write_time) OVER () , 0) AS io_time_pct_instance,
             CASE WHEN s.total_plan_time + s.total_exec_time - s.blk_read_time - s.blk_write_time - s.temp_blk_read_time - s.temp_blk_write_time >= 0
                  THEN s.total_plan_time + s.total_exec_time - s.blk_read_time - s.blk_write_time - s.temp_blk_read_time - s.temp_blk_write_time
             END AS cpu_time,
