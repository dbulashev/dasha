...
## Поиск по логам
- Поиск по логам PostgreSQL и пулера соединений в уже существующем хранилище — отдельная страница `/logs`. Кластеры Yandex MDB работают из коробки через MDB API, self-managed читают индекс OpenSearch/Elasticsearch, описанный в `log_search.sources`
