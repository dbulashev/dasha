...
- Метрики «Счётчики» и «Время»; режиму времени нужен `track_io_timing = on` для строк отношений и `track_wal_io_timing = on` для строк журнала предзаписи, и он доступен, если включена хотя бы одна из настроек
...
## Поиск по логам
- Поиск по логам PostgreSQL и пулера соединений в уже существующем хранилище — отдельная страница `/logs`. Кластеры Yandex MDB работают из коробки через MDB API, self-managed читают индекс OpenSearch/Elasticsearch или поток VictoriaLogs, описанные в `log_search.sources`
...
- Per-user rate limiting настраивается отдельно для каждого источника, чтобы платный облачный API и локальный индекс не делили один бюджет (`log_search.rate_limit`, отдельный лимит для админов)
- `GET /api/logs/check` (только админ) показывает раскрытое имя индекса или выражение LogsQL, найденные и недостающие поля маппинга и одну маскированную запись-образец

## Анализ логов
- Страница `/log-insights` за одно чтение окна показывает, чем заняты логи: доли событий по категориям (дедлоки, ожидания блокировок, отмены, чекпоинты, автовакуум, временные файлы, ошибки), частые шаблоны сообщений в каждой и пометка у категорий, чьи записи пришли всплеском
- Планы `auto_explain` из того же чтения сгруппированы по запросу и форме плана: количество, суммарное и максимальное время, p50/p95 и дерево плана с подсветкой находок — последовательное чтение большой таблицы, ошибка оценки строк, сортировка на диске, взрывной рост повторов узла
- Скан сохраняется снимком: ссылка открывает те же цифры, пока снимок живёт (сутки), а таблица групп листает весь снимок, не перечитывая хранилище логов
- Сравнение с базовым окном (предыдущий период, сутки или неделю назад, произвольное) перечисляет запросы, которым стало хуже: новая форма плана, пропавший индекс, выросший p95
- Из отчёта по запросам есть вход «Планы из логов» по `queryid`. Пустой результат назван по причине: `auto_explain` не загружен, `auto_explain.log_min_duration` равен -1, формат плана не разбирается, `compute_query_id` выключен
- Выключается глобально параметром `log_insights.enabled`
...
- [MCP-коннектор](mcp.md) (`dasha-mcp`): read-only MCP-сервер с диагностикой флота для AI-ассистентов (34 tools, 5 prompts)
