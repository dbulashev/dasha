...
- **Tools (34):** `list_clusters`, `fleet_health`, `get_instance_info`, `get_health_score`, `get_health_recommendations`, `health_details` (превращает рекомендацию в цель: передайте её `rule_id` как `detail` — вернутся сами таблицы, базы или сессии; потабличным drill-down нужна ещё `database`, инстанс-уровневым — нет), `health_trend`, `health_databases`, `top_queries` (по времени/WAL), `query_report`, `list_snapshots`, `query_compare`, `running_queries`, `blocked_queries`, `list_indexes` (missing/unused/usage), `unused_index_report` (вердикт по всему кластеру: можно ли удалить индекс — счётчик сканов взвешивается по всем хостам и по окну статистики, т.к. `idx_scan` не реплицируется, а счётчик без окна ничего не значит), `index_advisor` (обратная сторона того же вопроса — какой индекс создать: разбираются запросы со всех хостов кластера, и btree-кандидаты, которых не покрывает ни один существующий индекс, приходят с готовым DDL, стоящими за ними запросами и долей времени нагрузки, а также с признаками неполноты, по которым видно, когда пустой список — не «всё хорошо»; отчёт строится на лету, поэтому у него отдельный, больший таймаут), `top_tables`, `schema_lint` / `schema_lint_summary` (дефекты структуры схемы по системному каталогу: кончающиеся последовательности, таблицы без первичного ключа, unlogged-объекты, схемы, где может создавать объекты PUBLIC — со списком `skipped`, называющим невыполнившиеся проверки, чтобы пропуск не приняли за чистый результат), `hot_tables` / `hot_indexes` (топ горячих объектов по классам метрик — чтения/записи/io — из суточных дельта-снимков, просуммированных по всем хостам кластера, с coverage-долей репрезентативности топа; требует snapshot-хранилище), `describe_table`, `get_replication`, `settings_analyze`, `wait_events`, `connections`, `vacuum_danger`, `search_logs` (логи PostgreSQL/пулера любого кластера с источником логов — Yandex MDB, индекс OpenSearch или поток VictoriaLogs; с per-user rate limit), `plan_insights` / `query_plans` / `plan_regressions` (планы, которые `auto_explain` пишет в логи: одно чтение окна с разбивкой по категориям событий и группами планов с находками правил, планы одного запроса деревьями в духе EXPLAIN и запросы, чей план ухудшился между двумя окнами; каждый скан сохраняется под `scan_id`, и следующие вызовы читают его, не трогая логи; в планах только запуски дольше `auto_explain.log_min_duration`), `io_summary` / `io_trend` (физический I/O по `pg_stat_io` в разрезе типа бэкенда, объекта и контекста и его динамика — единственные инструменты, которые видят I/O автовакуума, чекпойнтера и walwriter, а не только клиентских бэкендов; нужен PostgreSQL 16+ и snapshot-хранилище). Все помечены **read-only** и closed-world, чтобы совместимые клиенты показывали (и авто-аппрувили) их как безопасные. Сервер также отдаёт **инструкции** по использованию, которые подсказывают модели, какой tool/prompt выбрать.
...
- **Resources (8):** встроенная база знаний, которую модель читает по запросу — `dasha://kb/health-rules` (каждое правило health score с порогами LOW/MED/HIGH и первыми действиями), `dasha://kb/schema-checks` (каждый код проверки схемы, его params и первое действие), `dasha://kb/index-advisor` (как читать кандидатов на индексы, коды предупреждений и признаки неполноты), `dasha://kb/log-plans` (как читать планы из логов: выборка медленного хвоста, неполное окно, причины пустого ответа, каждый код находки с порогом, причины регрессий), `dasha://kb/wait-events` (глоссарий wait events), `dasha://kb/pg-stat-io` (как читать счётчики `pg_stat_io`), `dasha://kb/workflow` (сценарии «жалоба → цепочка инструментов» и правила бережности к API). Для оператора есть `dasha://mcp/tool-stats`: по каждому tool число вызовов, размер ответа (средний, максимальный, гистограмма) и сколько раз ответ сузили или отклонили с момента запуска сервера; в HTTP-режиме каждый токен видит только свои вызовы.
...
- **Таймауты:** `--timeout` (по умолчанию 15s) ограничивает любой вызов API; `index_advisor`, замер раздувания в `describe_table` и сканы логов в `plan_insights`, `query_plans` и `plan_regressions` идут через `--slow-timeout` (по умолчанию 90s). Если поднят серверный `index_advisor.timeout`, поднимите и `--slow-timeout`.
- **Размер ответа:** `--max-result-bytes` (или `DASHA_MCP_MAX_RESULT_BYTES`, по умолчанию 65536) задаёт бюджет одного ответа tool; держите его ниже инлайн-лимита клиента. Бюджет действует для tools, которые сами сужают ответ; такой ответ начинается с блока `{"shaped":…}`: что свёрнуто и каким параметром получить полный вид. Сужают ответ: `describe_table`, `health_trend`, `list_clusters`, `plan_insights`, `plan_regressions`, `query_compare`, `query_plans`; если сужать уже нечего, действует потолок 256 КБ. У остальных потолок 256 КБ (или бюджет, если он больше), сверх него tool отказывает и называет, что уменьшить.
- **Неизвестные имена:** на 404 приходит, что именно не нашлось (`scope`: cluster, instance, database; object, если в вызове названа таблица, снимок, база или хост, которых список кластеров не перечисляет, и их нет либо эндпоинт выключен; feature, если эндпоинт выключен в конфигурации Dasha), и до пяти похожих имён. Ответ остаётся ошибкой, под подсказанным именем вызов не повторяется.
...
Сервер read-only (мутирующих эндпоинтов нет) и работает под non-root. Хардненинг: размер ответа tool ограничен (см. «Размер ответа» выше; в невалидный JSON он не режется); кэш серверов по токену хэширован и ограничен; токены не логируются. В общем HTTP-развёртывании ставьте за TLS; rate-limit обеспечивает вышестоящий per-identity лимитер Dasha (каждый PAT — отдельная личность), поэтому он действует и через passthrough.
