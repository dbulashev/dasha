...
- **Resources (7):** встроенная база знаний, которую модель читает по запросу — `dasha://kb/health-rules` (каждое правило health score с порогами LOW/MED/HIGH и первыми действиями), `dasha://kb/schema-checks` (каждый код проверки схемы, его params и первое действие), `dasha://kb/index-advisor` (как читать кандидатов на индексы, коды предупреждений и признаки неполноты), `dasha://kb/wait-events` (глоссарий wait events), `dasha://kb/pg-stat-io` (как читать счётчики `pg_stat_io`), `dasha://kb/workflow` (сценарии «жалоба → цепочка инструментов» и правила бережности к API). Для оператора есть `dasha://mcp/tool-stats`: по каждому tool число вызовов, размер ответа (средний, максимальный, гистограмма) и сколько раз ответ сузили или отклонили с момента запуска сервера; в HTTP-режиме каждый токен видит только свои вызовы.
...
- **Таймауты:** `--timeout` (по умолчанию 15s) ограничивает любой вызов API; `index_advisor` и замер раздувания в `describe_table` идут через `--slow-timeout` (по умолчанию 90s). Если поднят серверный `index_advisor.timeout`, поднимите и `--slow-timeout`.
- **Размер ответа:** `--max-result-bytes` (или `DASHA_MCP_MAX_RESULT_BYTES`, по умолчанию 65536) задаёт бюджет одного ответа tool; держите его ниже инлайн-лимита клиента. Бюджет действует для tools, которые сами сужают ответ; такой ответ начинается с блока `{"shaped":…}`: что свёрнуто и каким параметром получить полный вид. Сужают ответ: `describe_table`, `health_trend`, `list_clusters`, `query_compare`; если сужать уже нечего, действует потолок 256 КБ. У остальных потолок 256 КБ (или бюджет, если он больше), сверх него tool отказывает и называет, что уменьшить.
