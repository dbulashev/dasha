...
- **Размер ответа:** `--max-result-bytes` (или `DASHA_MCP_MAX_RESULT_BYTES`, по умолчанию 65536) задаёт бюджет одного ответа tool; держите его ниже инлайн-лимита клиента. Ответ сверх бюджета tool сужает сам, и такой ответ начинается с блока `{"shaped":…}`: что свёрнуто и каким параметром получить полный вид. Tool, который сужать не умеет, отказывает и называет параметр, который надо уменьшить. У `unused_index_report`, `hot_tables`, `index_advisor`, `connections`, `query_report`, `schema_lint`, `health_trend`, `list_clusters`, `query_compare` и `describe_table` пока потолок 256 КБ.
- **Неизвестные имена:** на 404 приходит, что именно не нашлось (`scope`: cluster, instance, database или feature, если эндпоинт выключен в конфигурации Dasha), и до пяти похожих имён. Ответ остаётся ошибкой, под подсказанным именем вызов не повторяется.
