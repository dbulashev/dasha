...
- **Таймауты:** `--timeout` (по умолчанию 15s) ограничивает любой вызов API; `index_advisor` идёт через `--slow-timeout` (по умолчанию 90s), поскольку его отчёт строится на лету и не кэшируется. Если поднят серверный `index_advisor.timeout`, поднимите и `--slow-timeout`.
- **Размер ответа:** `--max-result-bytes` (или `DASHA_MCP_MAX_RESULT_BYTES`, по умолчанию 65536) задаёт бюджет одного ответа tool; держите его ниже инлайн-лимита клиента. Бюджет действует для tools, которые сами сужают ответ; такой ответ начинается с блока `{"shaped":…}`: что свёрнуто и каким параметром получить полный вид. Сужают ответ: `describe_table`, `health_trend`, `list_clusters`, `query_compare`. У остальных потолок 256 КБ (или бюджет, если он больше), сверх него tool отказывает и называет, что уменьшить.
