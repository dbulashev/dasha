...
В любом режиме, кроме `none`, в продакшене включайте `auth.require_https: true`: тогда Dasha отклоняет запросы, пришедшие по открытому HTTP (`X-Forwarded-Proto: https` от прокси, терминирующего TLS, считается защищённым), а пока флаг выключен — бэкенд пишет предупреждение при старте. В примерах выше он опущен, потому что ломает локальный запуск на `http://localhost`.
