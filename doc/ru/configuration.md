...
`GET /api/logs/insights` берёт записи за выбранный интервал из того же источника, подсчитывает события
по категориям и группирует планы `auto_explain` по запросу и структуре плана. Таймаут и `rate_limit`
те же, что у `log_search`. Объём одного запроса ограничивает глобальная секция `log_insights`:
...
log_insights:
  ...
  max_plan_bytes: 2097152 # план больше этого размера учитывается, но не разбирается
  max_plans: 5000         # сколько планов разбирать за запрос
```

Если лимит исчерпан раньше, чем прочитан весь интервал, в ответе указан период, за который собраны
данные. Из VictoriaLogs записи читаются начиная с новых, из OpenSearch — начиная со старых.

Планы попадают в лог, если `auto_explain` загружен через `shared_preload_libraries` и в его
`log_format` указан text или json. Проверки по фактическому числу строк и времени работают только с
`log_analyze = on`. Литералы в планах не маскируются: скрываются только учётные данные, как и в
поиске по логам.

Ещё до `max_plan_bytes` длину записи ограничивает сама доставка: VictoriaLogs отбрасывает строки
длиннее `-insert.maxLineSizeBytes` (по умолчанию 256 КиБ, максимум 2 МБ), а плагин `tail` в Fluent Bit
при включённом `Skip_Long_Lines` пропускает строки длиннее `Buffer_Max_Size`. Если агент доставки
обрезает длинные строки, текстовые планы приходят обрезанными.
