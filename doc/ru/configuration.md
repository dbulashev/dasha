...
запросов, он выключен по умолчанию. Роль, под которой Dasha подключается, должна входить в
`pg_monitor` и иметь право подключения к каждой наблюдаемой базе:
...
расширения — в примере это `public`. Dasha выполняет `SELECT monitoring.reset_pgss()` без
аргументов и результат не читает, так что тип возврата не важен. Имя функции тоже подставляется как
есть и должно быть вида `schema.name` без кавычек; при неверном имени Dasha пишет предупреждение и
вызывает функцию самого расширения. Пока `enable_query_stats_reset` выключен, кнопки сброса на
странице нет.
