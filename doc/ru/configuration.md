...
Представление читает `pg_statistic` с правами владельца, и статистика видна даже роли,
...
Поток адресуется выражением LogsQL. Агент доставки должен раскладывать запись по
...
`timestamp` и `host` в пресетах нет: PostgreSQL их не пишет, их именует агент доставки, поэтому оба
задаются явно. В хранилище уходят только фильтры по severity и хосту; подстроки по сообщению,
базе и пользователю Dasha фильтрует сама, так что анализируемое поле `text` поиску не мешает.
В OpenSearch поля severity, хоста и `selector` сравниваются точно, поэтому должны быть проиндексированы как `keyword`;
...
Роль `query_id` необязательна. Пресеты `jsonlog` и `csvlog` привязывают её к одноимённому полю,
которое PostgreSQL заполняет при включённом `compute_query_id`; у `odyssey` и `pgbouncer` роли нет. Поток
без этого поля ищется как обычно, а проверка источника показывает роль среди недостающих.

Роль `sql_state` тоже необязательна: `csvlog` привязывает её к `sql_state_code`, `jsonlog` — к
`state_code`. Сводка по логам определяет категорию события сначала по коду SQLSTATE и только потом по
тексту сообщения.
...
`GET /api/logs/insights` читает логи кластера за выбранный интервал и собирает две сводки: число
событий по категориям (блокировки, контрольные точки, автоочистка, ошибки и другие) и планы
`auto_explain`, сгруппированные по запросу и структуре плана. Логи читаются из того же источника, что
и в поиске, с теми же `timeout_seconds` и `rate_limit`. Объём одного запроса ограничивает глобальная
секция `log_insights`:
...
Если лимит исчерпан до конца интервала, ответ указывает, какую часть интервала покрывает сводка: для
VictoriaLogs это последние записи, для OpenSearch первые. Если первым кончился `max_plans`, у планов
эта часть короче, чем у категорий.

Планы попадают в лог, когда `auto_explain` загружен через `shared_preload_libraries`, а
`auto_explain.log_format` равен `text` или `json`. В сводку попадают только запросы дольше
`auto_explain.log_min_duration`. Проверки плана по фактическому числу строк и времени работают только
при `auto_explain.log_analyze = on`. В планах маскируются только учётные данные, как в поиске по логам;
литералы запросов остаются.

Крупный план может не дойти до Dasha. VictoriaLogs отбрасывает строки длиннее
`-insert.maxLineSizeBytes` (по умолчанию 256 КиБ, максимум 2 МБ), плагин `tail` в Fluent Bit с
`Skip_Long_Lines` пропускает строки длиннее `Buffer_Max_Size`. Если агент доставки обрезает длинные
строки, текстовый план приходит без конца, а json-план не разбирается.
