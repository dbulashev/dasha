...
- **Result size:** `--max-result-bytes` (or `DASHA_MCP_MAX_RESULT_BYTES`, default 65536) is the budget of one tool result; keep it below the client's inline limit. A result over the budget is narrowed by the tool and opens with a `{"shaped":…}` block naming what was folded and the parameter for the full view; a tool that cannot narrow refuses and names the parameter to reduce. `unused_index_report`, `hot_tables`, `index_advisor`, `connections`, `query_report`, `schema_lint`, `health_trend`, `list_clusters`, `query_compare` and `describe_table` keep a 256 KB ceiling for now.
- **Unknown names:** a 404 answers with what did not resolve (`scope`: cluster, instance, database, or feature when the endpoint is disabled in Dasha's configuration) and up to five similar names; the call stays an error and is never repeated under a suggested name.
