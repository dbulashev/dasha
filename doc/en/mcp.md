...
- **Resources (8):** an embedded knowledge base the model can read on demand — `dasha://kb/health-rules` (every health rule with LOW/MED/HIGH thresholds and first actions), `dasha://kb/schema-checks` (every schema-check code, the params it fills and its first action), `dasha://kb/index-advisor` (how to read index candidates, warning codes and gaps), `dasha://kb/log-plans` (how to read plans from the logs: the slow-tail sample, partial windows, empty reasons, every finding code with its threshold, regression reasons), `dasha://kb/wait-events` (wait event glossary), `dasha://kb/pg-stat-io` (how to read the `pg_stat_io` counters), `dasha://kb/workflow` (complaint-to-tool-chain playbooks and API care rules). Plus `dasha://mcp/tool-stats` for the operator: per-tool call count, result size (average, maximum, histogram) and how often a result was narrowed or refused, since the server started; in HTTP mode each token sees only its own calls.
...
- **Timeouts:** `--timeout` (default 15s) bounds every API call; `index_advisor`, the bloat scan of `describe_table` and the log scans of `plan_insights`, `query_plans` and `plan_regressions` use `--slow-timeout` (default 90s). Raise `--slow-timeout` above the backend's `index_advisor.timeout` if that setting is raised.
- **Result size:** `--max-result-bytes` (or `DASHA_MCP_MAX_RESULT_BYTES`, default 65536) is the budget of one tool result; keep it below the client's inline limit. The budget applies to tools that narrow their own result: such a result opens with a `{"shaped":…}` block naming what was folded and the parameter for the full view. Narrowing tools: `describe_table`, `health_trend`, `list_clusters`, `plan_insights`, `plan_regressions`, `query_compare`, `query_plans`; a result already narrowed as far as it goes is held to the 256 KB ceiling instead. The rest keep a 256 KB ceiling (or the budget, if larger) and refuse above it, naming what to reduce.
- **Unknown names:** a 404 answers with what did not resolve (`scope`: cluster, instance, database; object when a name the cluster list cannot confirm (a table, a snapshot, or a database or host the cluster does not list) does not exist or the endpoint is disabled; feature when the endpoint is disabled in Dasha's configuration) and up to five similar names; the call stays an error and is never repeated under a suggested name.
...
The server is read-only (no mutating endpoints are exposed) and runs as a non-root user. Hardening: tool results are size-capped (see Result size above; never truncated into invalid JSON); the per-token server cache is hashed and bounded; tokens are never logged. Put the HTTP transport behind TLS in shared deployments; rate limiting is enforced upstream by Dasha's per-identity limiter (each PAT is a distinct identity), so it applies through the passthrough.
