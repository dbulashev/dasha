...
- **A tiny database is no longer scored as having a low cache hit ratio.** A database of a few small tables reads so few blocks that their first, cold reads alone drag the ratio down; the ratio is now graded only once the database has read enough to make it meaningful.
