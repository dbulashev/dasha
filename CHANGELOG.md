...
- **Hot tables & indexes (`GET /api/tables/hot`, `/api/indexes/hot`, `/api/hot/object-history`, `/api/hot/percentile`; MCP tools `hot_tables` / `hot_indexes`):** answers *what is loading this database* from scheduled activity-counter delta snapshots, stored the way PostgreSQL stores column statistics — an **exact top-N per metric class** (reads / writes / physical I/O; indexes have no writes class, PostgreSQL keeps no per-index write counters) plus a **decile histogram of the tail**, instead of either a full per-object history (grows with the customer's schema) or top-k-only time series (silently unrepresentative). The histogram buys three things raw tops cannot: a **coverage ratio** (what share of total activity the stored top actually holds — shown as a chip, so a top is never mistaken for the whole picture), a **percentile for any object** (a live delta against the stored anchor slice projected onto the tail distribution — the *Activity* block on the table describe page: absolute rate first, "hotter than ~N% of tables" second, because on a quiet database trivial activity outruns an idle tail), and a bounded storage footprint. Counters are sampled **from every cluster host** and summed (they are not replicated — the unused-index lesson applied to hot objects), with a per-host breakdown and primary/replica badges per top entry; an unreachable host degrades the snapshot to partial instead of blocking it. Capture runs in the autosnapshot daemon on a **cron schedule** (`hot_schedule`, default daily 03:00 **UTC** — pinned explicitly, since bare cron specs otherwise fire in whatever zone the backend container runs in; `CRON_TZ=<zone>` prefix supported), configurable along with top size and retention in the auto-snapshots settings UI; missed firings are taken on the next daemon tick. All rates are normalized by the snapshot's **actual window**, so hourly or 5-minute schedules extrapolate honestly to per-day figures — including the percentile, which scales the live rate back into window units before projecting. Storage is bloat-proof by construction: history tables are append-only day partitions dropped by their own age-based retention (`hot_retention_days`, independent of the size-based pgss retention), and the single rewritten table (the anchor slice) updates only non-indexed columns at fillfactor 70, so every daily rewrite stays HOT. UI: *Hot tables* / *Hot indexes* sections (class selector, snapshot picker, trend arrows against the previous snapshot, expandable per-host rows, tail summary with decile mini-bars), large numbers shortened with k/M/B suffixes. Requires snapshot storage (`501` otherwise); sections hide themselves when it is absent. Honesty rules for quiet databases: zero-activity objects never pad the top ("in the hot top" while doing nothing — observed on the demo lab, where 7 slots over 17 mostly-idle tables were filled with silence), a fully idle tail collapses to one phrase instead of nine zero-height bars, and an idle table's describe block folds into a single "no notable activity" line. The auto-snapshots page separates the two snapshot families: the settings tab groups triggered pgss/lock sections apart from the scheduled hot-objects section, and the status bar shows *last trigger event* and *last hot snapshot* as distinct facts. (`GET /api/maintenance/autovacuum-summary`):** a pie chart of tables currently past their autovacuum/autoanalyze trigger thresholds — split into *VACUUM only / ANALYZE only / both*, computed with PostgreSQL's own formula (`threshold + scale_factor × rows`, per-table reloptions honoured) — plus counters of maintenance processes currently running in the database (`pg_stat_progress_vacuum` / `pg_stat_progress_analyze`). A legend with counts and shares sits next to the chart (and is hidden entirely when nothing is due — a green "all within thresholds" state replaces the chart); slice colors are CVD-validated against light and dark surfaces separately.
