...
### Features
- **Log search works on self-managed clusters.** Logs are read from an existing OpenSearch or Elasticsearch index: one source serves the whole fleet, the index pattern carries the cluster name, and a field map — with presets for `jsonlog`, `csvlog`, Odyssey and pgbouncer — says which field holds which role. Filters, deduplication, masking and pagination behave exactly as on Yandex MDB clusters. The server must write `jsonlog` or `csvlog`; an index holding the raw log line is refused with a message instead of an empty result.
...
- **The level filter offers the levels of the bound log source**, the pgbouncer `NOISE` and `LOG` included.
...
- **The health score accounts for sequence exhaustion without a metrics datasource.** The recommendation is built from the schema checks, at the same thresholds, and links to the sequences behind it.
...
- **The Helm chart can publish Dasha through an existing Gateway** (`gatewayAPI.createGateway: false`): the chart renders only the `HTTPRoute` and attaches it to a shared Gateway that lives outside the chart with its own listeners and certificates.
...
- **Snapshot comparison renders in pages**, the way the query report does: a comparison covering more than a thousand statements used to freeze the page until every card was drawn.

### Bug Fixes
- **Fixed the timeout in the largest-tables report on databases with thousands of objects.**
- **A tiny database is no longer scored as having a low cache hit ratio.** A database of a few small tables reads so few blocks that their first, cold reads alone drag the ratio down; the ratio is now graded only once the database has read enough to make it meaningful. This needs the read counter the app collects itself; with a metrics datasource the ratio is graded the way it always was.
- **The "low cache hit ratio" recommendation opens the Tables page**, where the tables doing the reading are — it used to lead to index usage.
...
### Bug Fixes
- **Extension discovery** With `pg_stat_statements` or `pgstattuple` created outside the default `search_path` or different database, every query statistics page reported the extension as inaccessible.
- **A table held under a lock no longer answers "500 Internal Server Error".** Opening the description of a table locked by `ALTER TABLE`, `VACUUM FULL` or a transaction left open comes back at once and names the cause, instead of waiting out the query timeout.
...
- **Auto-snapshots missed genuine activity spikes.** Three things conspired to make the spike rule almost unreachable. The baseline is a moving average over `window_size`, so with a window as short as the required spike duration a sustained spike inflated its own baseline and fell back under the threshold before the duration elapsed — the more the load grew, the faster the reference grew with it. Second, real load is saw-toothed: a single probe dipping under the threshold reset the timer, so a spike interrupted once a minute never accumulated its minutes. Third, the shipped defaults (5-minute window, 5-minute duration) were exactly the degenerate combination. Now a dip shorter than half the required duration no longer aborts a forming spike, provided at least 70% of its probes stayed above the threshold — the achieved share is recorded as "Coverage" in the trigger history, next to the baseline and the peak. The window must now be **at least twice** the spike duration (rejected on save otherwise), and new installations start at a **30-minute window with a 2-minute duration** instead of 5m/5m. Existing settings are left untouched — if yours has a window at or below the spike duration, widen it: that combination cannot fire.

### Dependencies
- **Security updates in transitive Go modules:** grpc 1.79.3→1.82.1 (xDS RBAC and HTTP/2 vulnerabilities, GHSA-hrxh-6v49-42gf) and x/text 0.38.0→0.39.0 (infinite loop on invalid input, GO-2026-5970 — reachable through the pgx connection path, so not merely theoretical); genproto googleapis/api and /rpc refreshed alongside grpc.
